...
function Update() {
    ...
    {
    	movement.direction = Vector3(movement.horizontalDirection, 0, 0);
	}
  MoveCharacter();
}
    ...
    switch (controller.isGrounded) {
        ...
        case false:
            if (movement.enabled) {
                ...
                if (Input.GetButtonDown("Jump") && ! movement.usedExtraJump && dubblejump) {
              //  if (Input.GetAxisRaw("Vertical") == 1 && ! movement.usedExtraJump) {
...
function OnTriggerEnter (collid : Collider) {   
	...
	else if(collid.tag == "finish")
	{
		 var nextlvl : String = collid.GetComponent("finish").nextLVL ; 
		  ...
		  nextlvl = nextlvl.Substring(5,nextlvl.length-5);
		  print(nextlvl);
		...
		//check if hit player
	else if(collid.tag == "killDestroy" && movement.enabled)
	{
...
private function CreatePlatform () {
		Debug.Log("createplatform");
	 	if(!movement.usedExtraJump&&platformDoubleJump || !platformDoubleJump)
